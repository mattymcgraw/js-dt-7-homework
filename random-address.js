...
function randomizer(input){
	if (isNaN(input)){
		...
		var randomArray = input;
		randomArray[Math.floor(Math.random() * randomArray.length)];
	}

}

console.log((randomizer(streetNumber));+" "+(randomizer(streetName));+" "+(randomizer(streetType));+", "+(randomizer(cityName));+" "+(randomizer(stateAbbrev));+", "+(randomizer(zipCode));))

// console.log((randomizer(streetNumber)));